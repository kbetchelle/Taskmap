 ...
 *  - Shorter targets get a small bonus (tighter match)
 *
...
export function fuzzyMatch(query: string, target: string): FuzzyResult {
  ...
  if (t.startsWith(q)) {
    // Shorter targets score higher for the same prefix match
    const lengthBonus = Math.max(0, 20 - target.length)
    return { match: true, score: 100 + q.length + lengthBonus }
  }
  ...
  if (qi === q.length) {
    // Shorter targets get a small bonus (tighter match)
    const lengthBonus = Math.max(0, 10 - Math.floor(target.length / 10))
    return { match: true, score: score + lengthBonus }
  }
